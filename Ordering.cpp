...
namespace seneca {
   ...
   Ordering::~Ordering() {
      delete[] drinks;
      delete[] foods;
      for (size_t i = 0; i < billItemCount; ++i) delete billItems[i];
   }
   ...
   void Ordering::listFoods() const {
      ...
      for (size_t i = 0; i < foodCount; ++i) foods[i].print();
      cout << "========================================\n";
   }
   ...
   void Ordering::orderFood() {
      ...
      for (size_t i = 0; i < foodCount; ++i) menu << foods[i].getName();
      int sel = menu.select();
   ...
   void Ordering::orderDrink() {
      ...
      for (size_t i = 0; i < drinkCount; ++i) menu << drinks[i].getName();
      int sel = menu.select();
